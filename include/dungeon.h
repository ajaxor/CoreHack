...
#define Is_astralevel(x) (FALSE)   // Lcheck(x, &astral_level))
#define Is_earthlevel(x) (FALSE)   // Lcheck(x, &earth_level))
#define Is_waterlevel(x) (FALSE)   // Lcheck(x, &water_level))
#define Is_firelevel(x) (FALSE)    // Lcheck(x, &fire_level))
#define Is_airlevel(x) (FALSE)     // Lcheck(x, &air_level))
#define Is_medusa_level(x) (FALSE) // Lcheck(x, &medusa_level))
#define Is_oracle_level(x)  (Lcheck(x, &oracle_level))
#define Is_valley(x) (FALSE)        // Lcheck(x, &valley_level))
#define Is_juiblex_level(x) (FALSE) // Lcheck(x, &juiblex_level))
#define Is_asmo_level(x) (FALSE) // Lcheck(x, &asmodeus_level))
#define Is_baal_level(x) (FALSE) // Lcheck(x, &baalzebub_level))
#define Is_wiz1_level(x) (FALSE) // Lcheck(x, &wiz1_level))
#define Is_wiz2_level(x) (FALSE) // Lcheck(x, &wiz2_level))
#define Is_wiz3_level(x) (FALSE) // Lcheck(x, &wiz3_level))
#define Is_sanctum(x) (FALSE) // Lcheck(x, &sanctum_level))
#define Is_portal_level(x)  (Lcheck(x, &portal_level))
#define Is_rogue_level(x) (FALSE) // Lcheck(x, &rogue_level))
#define Is_stronghold(x)    (Lcheck(x, &stronghold_level))
#define Is_bigroom(x) (FALSE) // Lcheck(x, &bigroom_level))
...
#define Is_knox(x) (FALSE)          // Lcheck(x, &knox_level))
#define Is_mineend_level(x) (FALSE) // Lcheck(x, &mineend_level))
#define Is_sokoend_level(x) (FALSE)//Lcheck(x, &sokoend_level))

#define In_sokoban(x) (FALSE)//(x)->dnum == sokoban_dnum)
#define Inhell In_hell(&u.uz) /* now gehennom */
#define In_endgame(x) (FALSE)//(x)->dnum == astral_level.dnum)
